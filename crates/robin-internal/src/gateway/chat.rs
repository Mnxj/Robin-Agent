...
(function() {
	...
	toggleToolsBtn.addEventListener('click', function() {
		...
		localStorage.setItem('robin-hide-tools', toolsHidden);
		applyToolVisibility();
	});
	...
	var traceVisible = localStorage.getItem('robin-show-trace') === 'true';
	...
	toggleTraceBtn.addEventListener('click', function() {
		...
		localStorage.setItem('robin-show-trace', traceVisible);
		applyTraceVisibility();
	});
	...
	function setTheme(mode) {
		...
		localStorage.setItem('robin-theme', mode);
	}

	var saved = localStorage.getItem('robin-theme') || 'dark';
	setTheme(saved);
