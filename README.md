...
中文文档：[`README.zh-CN.md`](file:///Users/tianshi/Downloads/felix-main/felix-rust/README.zh-CN.md)

A self-hosted AI agent gateway written in Rust. Single binary, low memory, runs entirely on your own machine.

Robin connects you (via CLI or web chat) to LLMs — Claude, GPT, Gemini, Qwen, or any OpenAI-compatible endpoint — and lets agents execute tasks on your hardware using a fixed registry of in-process tools plus any number of remote MCP servers.

## Design philosophy

1. **Self-sufficient.** One binary, one directory of state, no required network dependency. The LLM can be local. The vector index is in-process. The knowledge graph is a SQLite file. There is no Robin cloud, no Robin account, no Robin backend that anyone could turn off.
...
- Claude, GPT, Gemini, Qwen, LM Studio, DeepSeek, or any OpenAI-compatible API.
...
- Config hot-reload: edit `robin.json5` while running, changes apply immediately.
...
Download the latest `Robin-vX.Y.Z-signed.pkg` from the [GitHub Releases](https://github.com/sausheong/robin/releases) page. Signed with Developer ID and notarized by Apple, so Gatekeeper accepts it.
...
To uninstall: `rm /usr/local/bin/robin && rm -rf /Applications/Robin.app ~/.robin/`.

### Build from source (Linux, Windows, or developers)

Requires [Rust](https://rustup.rs/) 1.75+. For Linux and Windows cross-compilation, install [cross](https://github.com/cross-rs/cross) (Docker-based):
...
docker build -t robin:local .                      # optional container image
```

Then run `./target/debug/robin onboard` to walk through provider setup.

## First steps

```bash
./robin onboard       # First-time setup wizard (writes ~/.robin/robin.json5)
./robin chat          # Interactive CLI chat
./robin start         # Run the gateway (web chat at http://127.0.0.1:18789/chat)
open Robin.app        # macOS tray launcher (auto-spawns the gateway)
./robin doctor        # Sanity-checks config, data dirs, API keys, workspaces
```

`./robin chat` automatically detects a running gateway and shares its session state with the web chat. Pass `--no-gateway` to force an isolated in-process runtime; pass `-m provider/model` to override the model for one session (also forces in-process).

Inside `robin chat`, slash commands manage sessions and screenshots:
...
| Command                               | Description                                          |
| ------------------------------------- | ---------------------------------------------------- |
| `robin onboard`                       | Interactive setup wizard                             |
| `robin start`                         | Start the gateway server                             |
| `robin start -c path/to/config.json5` | Custom config                                        |
| `robin chat [agent]`                  | Interactive CLI chat (auto-detects running gateway)  |
| `robin chat --no-gateway`             | Force in-process runtime, ignore any running gateway |
| `robin chat -m provider/model`        | Override model for this session (forces in-process)  |
| `robin clear [agent]`                 | Clear local CLI session history                      |
| `robin sessions [agent]`              | List sessions for an agent                           |
| `robin mcp login <id>`                | Run interactive OAuth login for an MCP server        |
| `robin status`                        | Query the running gateway for agent status           |
| `robin doctor`                        | Diagnostic checks                                    |
| `robin version`                       | Print version + commit                               |

## System tray app

A thin launcher around the gateway, on macOS and Windows. Spawns `robin start` as a separate child process so a tray reap (display sleep, fast user switching, memory pressure) doesn't take your active chat down — only the icon disappears, and relaunching reattaches via `/health`.

Menu: **Chat**, **Jobs**, **Logs**, **Settings**, **Restart**, **Quit**.

The Settings page has tabs for Agents, Providers, Models, Intelligence, Security, Messaging, MCP, Skills, Memory, and Gateway — most things you'd otherwise edit in `robin.json5` are reachable here.
...
**Environment variables.** macOS `.app` bundles don't inherit shell environment variables; Robin.app loads `~/.zshrc` / `~/.bashrc` at startup, so `export ANTHROPIC_API_KEY=...` works. On Windows, set via System Settings or PowerShell `[System.Environment]::SetEnvironmentVariable("ANTHROPIC_API_KEY","sk-ant-...","User")`. Either way, you can put keys directly in `robin.json5` instead.

## Configuration

`~/.robin/robin.json5` (Windows: `C:\Users\<you>\.robin\robin.json5`). JSON5 means comments and trailing commas are allowed. Hot-reloaded — edits apply immediately, no restart needed.
...
Robin supports multiple providers simultaneously. Each is defined in the `providers` block with a unique name and a `kind`:
...
| `openai-compatible` | Anything implementing `/v1/chat/completions` | LM Studio, DeepSeek, LiteLLM, vLLM, Ollama |
...
// Models: any tag pulled into Ollama (llama3.2, qwen2.5, mistral, ...)
```
...
ollama/llama3.2                → uses the "ollama" openai-compatible provider
deepseek/deepseek-chat         → uses the "deepseek" provider
```

You can override the model for one CLI session: `robin chat -m openai/gpt-5`.
  ...
  "agents": {
    "list": [
      ...
      },
      {
        ...
        "name": "Researcher",
        "model": "openai/gpt-5",
        "workspace": "~/.robin/workspace-researcher",
...
Chat with a specific agent: `robin chat coder` (or pick from the dropdown in the web chat header).
...
{
  "id": "coder",
  "system_prompt": "You are a senior Rust developer. Idiomatic, safe, zero-cost abstractions. Always write tests."
}
```

Or in `~/.robin/workspace-coder/IDENTITY.md` for prompts long enough to be uncomfortable in JSON.
...
Robin can push messages to a Telegram chat via the Bot API. **Outbound only** — there is no inbound Telegram channel.
...
Static cron jobs go in `agents[].cron[]` in the config and persist across restarts. Dynamic jobs added via the tool also persist (`~/.robin/cron-jobs.json`). Both use the same scheduler. Schedule values are duration strings (`30m`, `1h`, `24h`).

### MCP servers

Robin can connect to external [Model Context Protocol](https://modelcontextprotocol.io) servers and expose their tools alongside built-ins. Two transports: **HTTP** (Streamable HTTP, with OAuth2 / bearer / no auth) and **stdio** (Robin spawns the child process).
...
```json5
{
  "mcp_servers": [
    ...
    // Initial login: `robin mcp login user-tools`
    ...
    // Expiry mid-chat: inline "Re-authenticate" button in the web UI
    {
      ...
      "http": {
        ...
        "auth": {
          ...
          "token_store_path": "~/.robin/mcp-tokens/user-tools.json"
        }
      }
    },

    // Stdio: Robin spawns the child process, inherits PATH
    {
      "id": "fs-tools",
      "transport": "stdio",
      "enabled": true,
...
Skills are Markdown files with YAML frontmatter. Robin injects only the index (name/description/tags) into the prompt, and the model fetches full bodies on demand via `load_skill`.
...
- `~/.robin/skills/` — shared across all agents
- `<agent-workspace>/skills/` — agent-specific

**How they're matched.** Robin injects only the *index* (name + description + tags) into every turn's system prompt. The model decides when it needs a skill body and calls the `load_skill` tool to fetch it on demand. This keeps the cached prompt prefix small and stable, which keeps prompt caches hitting.
...
Memory entries are Markdown files in `~/.robin/memory/entries/`. The runtime performs BM25 recall each turn (optional vector search when configured), injects relevant snippets, and also exposes a Memory Index so the model can lazily hydrate full entries via `load_memory`.

Memory entries are manageable from Settings via `GET/POST/DELETE /settings/api/memory*`.

You can manually drop entries in too:

```bash
cat > ~/.robin/memory/entries/project-conventions.md << 'EOF'
# Project Conventions
- Rust edition 2021
- axum for HTTP handlers
- tokio for async runtime
- Errors wrapped with anyhow or thiserror
EOF
```

If you also configure an `embeddingProvider` and `embeddingModel` under `memory`, vector search runs alongside BM25.
...
Robin is designed to run on your own hardware. The defaults protect you from the common ways an agent with broad capabilities can go wrong; you relax them deliberately, not opt out of them.
    ...
    "list": [
      {
        ...
        "model": "anthropic/claude-sonnet-4-5",
        "workspace": "~/.robin/workspace-assistant",
        "tools": {
          "allow": ["read_file", "write_file", "edit_file", "bash",
  ...
  "agents": {
    "list": [
      ...
      },
      {
        ...
        "model": "openai/gpt-5-mini",
        "subagent": true,
        "description": "Fast lookups: man pages, syntax checks, single-file reads.",
...
### External Ollama (no cloud API keys)
...
```json5
{
  ...
  "agents": {
    "list": [
      { "id": "default", "model": "ollama/gemma3:4b",
...
Single-process, hub-and-spoke. All components run in one binary, powered by the tokio async runtime.

- **Gateway** — `axum` HTTP router + `tokio-tungstenite` WebSocket on `:18789`.
...
- **LLM provider abstraction** — one `LlmProvider` trait, five implementations (`anthropic`, `openai`, `gemini`, `qwen`, `openai-compatible`).
- **Session manager** — append-only JSONL with a DAG view; splice-based compaction.
- **Memory manager** — BM25 always present, vector search optional.
...
│   │       └── mcp_cmd.rs      # robin mcp subcommands
│   ├── robin-app/              # System tray launcher (robin-app)
│   │   └── src/main.rs
│   └── robin-internal/         # Shared library (gateway, agents, tools, providers)
│       └── src/
│           ├── lib.rs
│           ├── config/
│           ├── gateway/
...
All state lives in `~/.robin/` (Windows: `C:\Users\<you>\.robin\`):
...
cargo test --workspace -- --nocapture              # verbose tests
cargo clippy --workspace -- -D warnings            # clippy
cargo fmt --all                                    # format
cargo check --workspace                            # type-check
./scripts/build-all.sh                             # build release artifacts into dist/
TAG=v0.1.0 ./scripts/publish.sh                    # publish dist/* to GitHub Release
...
| Markdown rendering (CLI)         | `pulldown-cmark`                                             |
| Terminal UI                      | `crossterm`                                                  |
| Date/time                        | `chrono`                                                     |
| UUID generation                  | `uuid`                                                       |
| SQLite                           | `rusqlite`                                                   |
| Parking lot sync primitives      | `parking_lot`                                                |
| Cross-platform cross-compilation | [`cross`](https://github.com/cross-rs/cross) (external tool) |
